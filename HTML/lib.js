...
window.addEventListener("noolsModelLoaded", (e) => {
	...
	if (!CTAT.ToolTutor.tutor.getOutputStatus().mustRetrieveProblemState()) {
		...
		CTATCommShell.commShell.processComponentAction(sai,true,true);	
	}
});

function __createElement(eStr) {
...
function __onload() {
	...
	//domain browser data
	var domainBrowserConfig = {};
	
	var problemValuesConfig = {};
	
	var reactionMenuConfig = {
		"tutor": ['reaction_menu_item']
	}
	
	//algebra parser
	var chemParser = new CTATChemParser();
	
	function addSubscriptsToChemFormula(formulaStr) {
	...
	*/
	function PopupMenu(containerName, options, parentMenu) {
		...
		*/
		...
		options.forEach((opt, i) => {
			...
			optDiv.id = this.container.id+"_option_"+optId;
			optDiv.classList.add("popup_menu_option");
			if (opt.customHTML) {
	...
	function QuantitySelector() {
		...
		function init() {
			div = __createElement("div.q_select.can_hide");
			tabRow = __createElement("div.q_select_tab_row");
			mainPane = __createElement("div.q_select_main_pane");
		...
		function populate(tabs) {
			...
			tabs.forEach((tab,idx)=>{
				var tabLabel = __createElement("div.q_select_tab");
				...
				var tabPane = __createElement("div.q_select_tab_pane");
				var propertyCol = __createElement("div.q_select_tab_pane_prop_col");
				var properties = {};
				tab.properties.forEach((p)=> {
					var pSelect = __createElement('select.q_select_prop_select');
					var opts = p.options.slice();
					opts.unshift("--"+p.name+"--");
					opts.forEach((o)=>{
						var oTag = __createElement('option');
						oTag.innerHTML = o;
				...
				var listPane = __createElement("div.q_select_list_pane");
			...
			var tabRowFiller = __createElement("div.q_select_tab_row_filler");
		...
		function listThese(qtyList) {
			...
			qtyList.forEach((q)=>{
				var qName = q.getName(),
					btnID = "qty_btn_"+q.id;
					qBtn = __createElement("div#"+btnID+".CTATButton.q_select_qty_btn");
	...
	*/
	function GuppyBox(div, render, doneOnBlur) { 
		...
		var active = false;
		var hiddenTextInput = __createElement("div#"+ctatId+".CTATTextInput");
	...
	/*	
		if (doneOnBlur) {
				console.log("add unfocus event");
				
				events["blur"] = () => {
					console.log("guppy "+div.id+" blur ");
					if (active) {
						workspace.triggerGuppyDone(ctatId);
						active = false;
					} else {
						console.log("not active, skipped");
					}
				};
			}
	*/		
	...
	*/
	function FormulaInput(id, labelPlaceholder) {
		var inputWrapper = document.createElement("div"),
			gup;
		inputWrapper.id = id;
		inputWrapper.classList.add("formula_entry_wrapper");
		this.domNode = inputWrapper;
		
		//label field
		var input1 = __createElement('div#'+id+"_name_entry.formula_entry_name.CTATTextInput");
		...
		//guppy field
		var input2 = __createElement('div#'+id+"_formula_entry.formula_entry_content");
		
		//colon b/w fields
		var eqSpan = __createElement("span.formula_entry_equals");
		eqSpan.innerHTML = ":";
		
		//cancel btn
		var cancelBtn = __createElement("div.close_btn");
		...
		//confirm btn
		var confirmBtn = __createElement("div.confirm_btn");
	...
	function Workspace(divId) {
		...
		**/
		...
		this.toPrecision = function(expr, numSigFigs) {
			...
			return chemParser.chemToPrecision(expr, numSigFigs, true);
		}
		...
		this.initSymbolListener = function() {
			window.addEventListener("keydown", (e) => {
				...
				let keyStr = e["ctrlKey"] ? "ctrl+" :
							 ...
							 e["altKey"] ? "alt+" : '';
				keyStr += e.key;
				console.log("keydown listener, keyStr is "+keyStr);
		...
		*/
		this.addExpression = function(expression) {
			...
			expressionContainer.appendChild(expression.domNode);
		...
		*/
		...
		this.formulaDeleted = function(expId) {
			console.log("tableDeleted "+expId);
		...
		this.doneEditingFormula = function(newStr, newLabel, wasSimped) {
			var expr = expRegistry[editingFormula],
				gup = expr.getGupInput(),
				gupWrapper = gup.getDomNode().parentNode;
			//set expr.string
			expr.setString(newStr, wasSimped);
			//set expr.label
		...
		this.variableEntered = function(newExpString, newExpId) {
			console.log("workspace.variableEntered()");
			var newExp = Expression.create(newExpString, currentVarReplace.getLabel(), newExpId)
			console.log("got newExp:",newExp);
			currentVarReplace.hideVarReplaceControls();
			pointer.addExpression(newExp);
			pointer.setButtonsLocked(false);
			currentVarReplace = null;
		}
		
		this.claimAdded = function(claimType) {
			var claim = createClaim.apply(this, arguments);
...
/*	
		//init popup menu
		popupMenu = new PopupMenu("var_popup_menu",
						[
							{
								name:"replace",
								handler: menuClicked.bind(this, "replace")
							},
							{
								name: "calculate",
								handler: menuClicked.bind(this, "calculate")
							},
							{
								name: "enter",
								handler: menuClicked.bind(this, "enter")
							}
						]
					);
		
		//listener for clicks off the popup menu
		document.body.addEventListener("click", (e) => {
			if (!e.target.className.includes("var_popup_option")) {
				this.hidePopup();
			}
		});
*/
	...
	var Expression = (function(){
		...
		*/
		...
		function Expression (expString, optLabel, id, gup, wasSimped, ogStr) {
			...
			*/
			this.setString = function(newStr, wasSimplified) {
				var gupXml;
				var processed = GuppyUtil.processString(newStr);
				expString = processed.text;
				if (gupInput) {
					gupXml = gupInput.getXml();
				} else if (wasSimplified) {
					gupXml = GuppyUtil.processString(ogStr).xml
				}
				...
				variables = chemParser.chemGetVariables(expString);
			...
			*/
			...
			this.simplify = function() {
				...
				return new Expression(chemParser.chemSimplify(pointer.toString(), false, true), optLabel, expId+"_simplified");
			};
			
			this.solveQuadratic = function() {
				if (!roots) {
					roots = chemParser.chemSolveQuadratic(...quadraticCoefficients);
			...
			this.replaceVar = function(varName, varValue, newExpId) {
				var newStr = chemParser.chemStringify(chemParser.chemSetVariable(pointer.toString(), varName, varValue), true);
			...
			this.toggleMenu = function(forceState) {
				switch(forceState) {
					...
					default:
						optionMenu.toggle();
						forceState = optionMenu.isOpen() ? "open" : "close";
				}
			};
			...
			function initOptionMenu() {
				...
				let optionMenuDiv = __createElement('div#'+expId+"_option_menu.expression_option_menu");
				...
				if (opts.indexOf("solve quadratic") > -1) {
					...
					quadraticCoefficients = chemParser.chemGetQuadraticCoefficients(expString, true)
				}
				if (opts.indexOf("simplify") > -1) {
					let simpArdy = chemParser.chemSimplified(expString);
					optionMenuBtns.push({
						name: "simplify",
						handler: null,
						isCTAT: true,
				...
				if (opts.indexOf("replace variable") > -1) {
					...
					optionMenuBtns.push({
						...
						closeOnClick: true,
						handler: ()=>{
							console.log("replace var handler");
							if (!workspace.getCurrentVarReplace()) {
								console.log("replace var handler 2");
				...
				if (opts.indexOf("set unit") > -1) {
					...
					var unitRowHtml = units.map((u)=>"<div id='"+expId+'_set_unit_'+u.str+"' class='CTATButton popup_menu_option_row_btn' data-ctat-disable-on-correct='false' data-ctat-show-feedback='false'>"+u.html+"</div>").join('');
					optionMenuBtns.push({
						name: "set unit",
						customHTML: "<div id='"+expId+"_unit_option_row' class='popup_menu_option_row'>"+
					...
					*/
					...
					unitDisplay = __createElement('div#'+expId+"_unit_display.expression_unit_display.can_hide");
				...
				if (opts.indexOf("set substance") > -1) {
					...
					optionMenuBtns.push({
						name: "set substance",
						customHTML: "<div id='"+expId+"_substance_option_row' class='popup_menu_option_row'>"+
										"<div class='popup_menu_option_row_title'>set substance:</div>"+
										subs.map((s)=>"<div id='"+expId+'_set_substance_'+s.str+"' class='CTATButton popup_menu_option_row_btn' data-ctat-disable-on-correct='false' data-ctat-show-feedback='false'>"+s.html+"</div>").join('')+
									"</div>",
						handler: function(e) {
							e.preventDefault();
							e.stopPropagation();
					...
					*/
					...
					substanceDisplay = __createElement('div#'+expId+"_substance_display.expression_substance_display.can_hide");
					contentWrap.appendChild(substanceDisplay);
				}
				
				if (opts.indexOf("set context") > -1) {
					var ctxtOpts = ['changed', 'final', 'equilibrium'],
					...
					optionMenuBtns.push({
						name: "set context",
						customHTML: "<div id='"+expId+"_context_option_row' class='popup_menu_option_row'>"+
										"<div class='popup_menu_option_row_title'>set context:</div>"+
										ctxtOpts.map((c)=>"<div id='"+expId+'_set_context_'+c+"' class='CTATButton popup_menu_option_row_btn' data-ctat-disable-on-correct='false' data-ctat-show-feedback='false'>"+c+"</div>").join('')+
									"</div>",
						handler: function(e) {
							e.preventDefault();
							e.stopPropagation();
					...
					*/
					...
					contextDisplay = __createElement('div#'+expId+"_context_display.expression_context_display.can_hide");
			...
			menuBtn.id = expId+"_open_menu_btn";
			menuBtn.classList.add("expression_btn");
			menuBtn.innerHTML = "options";
			...
			deleteBtn.classList.add("expression_btn");
			deleteBtn.classList.add("close_btn");
			deleteBtn.classList.add("CTATButton");
			deleteBtn.id = expId+"_close_btn";
	...
	function Table(tableId, reactionStr, isEquilibrium, inactiveReagents, initialRows) {
		...
		var leftCols = sides[0].split('_plus_').map((sub)=>{
			return {text: sub, html: addSubscriptsToChemFormula(sub), substance: sub.replace(/^\d/, '')};
		});
		...
		*/
		...
		this.cellEntered = function(cellId, guppy, str) {
			...
			contentDiv.classList.add("reaction_table_cell");
			contentDiv.classList.add("reaction_table_value_entered");
			contentDiv.classList.add("no_select");
		
			//simplified guppy output
			let simpGuppy = GuppyUtil.processString(str);
	...
	var createClaim = (function(){
		
		var finalQtySerial = 0;
		
		function claimBase(claimId, guppyIds) {
			this.domNode = document.createElement('div');
			this.id = this.domNode.id = claimId;
			this.domNode.classList.add("claim_container");
			this.domNode.classList.add("expression");
			this.submitBtn = document.createElement('div');
			this.submitBtn.id = claimId+"_submit_btn";
			this.submitBtn.classList.add("CTATButton");
			...
			guppyIds.forEach((id, idx)=>{
				var guppyDiv = document.createElement("div");
				guppyDiv.id = this.domNode.id+id;
				guppyDiv.classList.add("claim_guppy");
				this.guppies[idx] = this.guppies[guppyDiv.id] = new GuppyBox(guppyDiv, false, true);
				this.guppies.length++;
			});
			
			this.render = function() {
				for (let i = 0; i < this.guppies.length; i++) {
					this.guppies[i].render();	
				}
			};
			
			this.valueEntered = function(inputId, valStr) {
				var guppy = this.guppies[inputId],
					guppyDiv = guppy.getDomNode(),
					nextSib = guppyDiv.nextSibling,
					dispNode = this.valDisplayNodes[inputId],
					processed = GuppyUtil.processString(valStr);
				
				if (!dispNode) {
					dispNode = document.createElement('div');
					dispNode.id = inputId+"_display_node";
					dispNode.classList.add("claim_value_display_node");
					dispNode.addEventListener('dblclick', this.editValue.bind(this, dispNode, guppy));
					dispNode.style.minWidth = $(guppyDiv).width()+'px';
					this.valDisplayNodes[inputId] = dispNode;
				}
			...
			this.editValue = function(dispNode, guppy) {
		...
		var claimConstructors = {
			'limitingReagent': function(id, reagentList, studentValList) {
				claimBase.call(this, id, ["_value_input_1", "_value_input_2"]);
				var reagentOptions = ['---'].concat(reagentList.map((ro)=>ro.substance));
				var substanceMenu = createComboBox(
				...
				var comparisonMenu = createComboBox(
				...
				this.domNode.insertBefore(this.guppies[1].getDomNode(), this.submitBtn);
				this.domNode.insertBefore(comparisonMenu, this.guppies[1].getDomNode());
				this.domNode.insertBefore(this.guppies[0].getDomNode(), comparisonMenu);
				this.domNode.insertBefore(tNode2, this.guppies[0].getDomNode());
			...
			'finalQuantity': function(id, finalQtyList, studentValList) {
				...
				this.domNode.insertBefore(unitField, this.submitBtn);
				this.domNode.insertBefore(this.guppies[0].getDomNode(), unitField);
				this.domNode.insertBefore(textNode2, this.guppies[0].getDomNode());
			...
			'validateXAssumption': function(id) {
				...
				var percentMenu = createComboBox(
				...
				var tNode2 = document.createTextNode(" because ");
				this.domNode.insertBefore(percentMenu, this.submitBtn);
			...
			'phOfSolution': function(id) {
				...
				this.domNode.insertBefore(this.guppies[0].getDomNode(), this.submitBtn);
				this.domNode.insertBefore(tNode1, this.guppies[0].getDomNode());
			},
	...
	function createComboBox(id, classes, options, tutored, feedback, lockOnCorrect) {
		var combo = document.createElement('div');
		combo.id = id;
		combo.classList.add("CTATComboBox");
		classes.forEach((cl) => {
			combo.classList.add(cl);
		});
	...
	window.__uiInit = function(problemData, expOptions) {
		...
		var claimMenuTypes = problemData.claimTypes,
			symbolList = problemData.symbols,
		...
		Guppy.prototype.typeInString = function(string) {
			this.typeInTree(chemParser.chemParse(string));
		};
		
		
		//Guppy init
		let pathArr = scriptSrc().split('/');
		pathArr.pop();
		...
		GuppyUtil = {
			...
			*/
			processString: function(string) {
				...
				let tree = chemParser.chemParse(string);
		...
		/** init tree menus **/
		console.log("create domain browser");
		var domainBrowserMenu = new TreeMenu("domain_browser_popup", "Domain Browser", domainBrowserConfig, "workspace_scrim");
		domainBrowserMenu.setClickHandler((itemClicked) => {
			if (itemClicked.type === "domain_browser_formula") {
				domainBrowserMenu.hide();
			}
		});
		
		console.log("create data browser");
		var dataBrowserMenu = new TreeMenu("problem_values_popup", "Problem Values", problemValuesConfig, "workspace_scrim");
		dataBrowserMenu.setClickHandler((itemClicked) => {
			if (itemClicked.type === "value") {
				workspace.replaceVar(itemClicked.value /* + ' ' + itemClicked.units */);
				dataBrowserMenu.hide();
			}
		});
		
		console.log("create reaction browser");
		reactionMenuConfig.items = reactionList;
		var reactionMenu = new TreeMenu("reaction_popup", "Chemical Reactions", reactionMenuConfig, "workspace_scrim");
		...
		var claimMenuConfig = {
			...
			'items': claimMenuTypes.map((cmt) => {
				return typeof cmt === "string" ? {
		...
		var claimMenu = new TreeMenu("claim_popup", "Claim Types", claimMenuConfig, "workspace_scrim");
		...
		/** init buttons **/
/*		
		var addFormulaButton = document.querySelector("#add_formula_btn");
		addFormulaButton.addEventListener("click", () => { 
			if (!addFormulaButton.className.includes('locked')) {
				domainBrowserMenu.show();
			}
		});

		var dupFormulaButton = document.querySelector("#dup_formula_btn");
		dupFormulaButton.addEventListener("click", () => { 
			if (!dupFormulaButton.className.includes('locked')) {

			}
		});
*/		
		var workspaceBtnWrapper = document.querySelector("#workspace_btn_row");
		var makeClaimButton = document.createElement('div');
		makeClaimButton.id = "make_claim_btn";
		makeClaimButton.innerHTML = "Make Claim";
		makeClaimButton.classList.add("CTATButton");
		makeClaimButton.classList.add("workspace_btn");
		...
		workspaceBtnWrapper.appendChild(makeClaimButton);
		
		if (problemData.usesIceTable) {
			var createTableButton = document.createElement("div");
			createTableButton.id = "create_table_btn";
			createTableButton.innerHTML = "Create ICE Table";
			createTableButton.classList.add("CTATButton");
			createTableButton.classList.add("workspace_btn");
			...
			workspaceBtnWrapper.appendChild(createTableButton);
		}
		
		var createFormulaButton = document.createElement('div');
		createFormulaButton.id = "create_formula_btn";
		createFormulaButton.innerHTML = "Create Formula";
		createFormulaButton.classList.add("CTATButton");
		createFormulaButton.classList.add("workspace_btn");
		...
		workspaceBtnWrapper.appendChild(createFormulaButton);
		
		//window mousedown event 
		window.addEventListener('mousedown', (e)=>{
			let target = (e.target.tagName.toLowerCase() === "button") ? e.target.parentNode : e.target;
			if (!target.className.includes("popup_menu_option")) {
			...
			} else if (activeGuppy && activeGuppy.doneOnBlur && activeGuppy.getDomNode() !== target && !target.className.includes('symbol_btn')) {
				//trigger done on activeGuppy if necessary
				workspace.triggerGuppyDone(activeGuppy);
				workspace.setActiveGuppy(null);
			}
		});
...
function genRuleChain(qty, chainData) {
	...
	if (!chainData.added[qty.id]) {
		...
		facts.forEach((fact)=> {
			if (!fact.matchedTo.length) {
				genRuleChain(fact, chainData);
			}
		});
		chainData.chain.push([rule, facts]);
	}
	
	return chainData.chain;
}

var repeatedExp = (function(){
	var expMap = {};
	return {
		clearMap: function() {
			expMap = {};
...
function getDistToQty(atQty, toQty, dist) {
	dist = dist || 0;
	if (atQty === toQty) {
		return dist;
	} else {
		...
		for (let i = 0; i < atQty.toFacts.length; i++) {
			let thisDist = getDistToQty(atQty.toFacts[i], toQty, dist);
			if (thisDist < min) {
				min = thisDist;
			}
		}
...
function getNextQtyForward(fromQty, destQty) {
	...
	if (destQty) {
		for(let i = 0; i < nextQtys.length; i++) {
			let thisDist = getDistToQty(nextQtys[i], destQty, 1);
			if (thisDist < dist) {
				nextQty = nextQtys[i];
				dist = thisDist;
			}
...
function getNextQtyBackward(targetQty, depth) {
	depth = depth || 0;
